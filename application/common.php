...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $level = Db::table('tp_user_level')->where(['level_id'=>$user['level']])->find();

    if($level != null){
        $result['order_amount'] =   $level['discount']*0.01*$result['total_amount'];
    }

    $fx_list = Db::table('tp_system_fx_config')->where(['level'=>1])->find();
    $result['fx_amount']    =   $fx_list['discount']*0.01*$result['order_amount'];

    $result['order_prom_amount']    =      $result['total_amount']-$level['discount']*0.01*$result['total_amount'];
