...
class MessageLogic extends Model
{
    ...
    public function getUserPerTypeLastMessage()
    {
        ...
        foreach ($categorys as $c) {
            $query = Db::query('SELECT m.category,m.message_id,um.status,m.send_time,m.type,m.data FROM tp_message m '
                    . 'INNER JOIN tp_user_message um ON (um.message_id=m.message_id AND um.user_id = ?) '
    ...
    public function getUserMessageList($user_id, $category, $p = 1)
    {
        ...
        $data = Db::query('SELECT m.category,m.message_id,um.status,m.send_time,m.type,m.data FROM tp_message m '
                    . 'INNER JOIN tp_user_message um ON (um.message_id=m.message_id AND um.user_id = ?) '
                    . 'WHERE m.type = 0 AND m.category = ? AND m.data!=""  '
                    . 'UNION (SELECT m.category,m.message_id, 1 AS status,m.send_time,m.type,m.data FROM tp_message m '
                    . 'WHERE m.type = 1 AND m.category = ? AND m.data!="") '
