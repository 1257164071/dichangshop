...
class UsersLogic extends Model
{
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $leader_data = M('users')->where(['user_id' =>  $_GET['first_leader']])->find();

            if($_GET['first_leader'])
                $map['first_leader'] = $_GET['first_leader']; // 微信授权登录返回时 get 带着参数的
                $map['user_pid']    =   $_GET['first_leader'];
            ...
            $user_tree_text  =   $leader_data['user_tree_text'].$row_id.'&';
            $user_else_data['user_tree_text']   =   $user_tree_text;
            $user_tree_arr   =   explode('&',$user_tree_text);
            $user_else_data['user_level']   =   count($user_tree_arr)-1;
            $res = M('users')->where(['user_id'    =>  $row_id])->update($user_else_data);
